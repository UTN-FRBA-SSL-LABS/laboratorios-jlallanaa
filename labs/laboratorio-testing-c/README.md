...
> R:Es importante ya que es la funcion que setea el contador de elementos del carrito en cero . No seria consistente que un usuario ya tenga elementos antes de utilizar el carrito, pudiendo estos tener, ademas, contenido basura.
...
> R: El test falla. Esperaba el valor 700 y obtuvo 350.
...
> R:Línea 24. Por cada iteración, al valor de total se le suma el valor de cada item mediante total += c->items[i].precio;

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:Además de tener en cuenta el valor del producto, se debe contar la cantidad de ese producto comprado, para obtener el total real. 
...
> R:El test falla, significa que carrito_agregar devuelve 1 con el quinto producto
...
> R:El operador incorrecto es el (<=), deberia ser menor estricto (<).
...
> R:La cobertura solo tuvo en cuenta el archivo de test unitarios, en los que no se llamó nunca a carrito_descuento. 
...
> R:Un test unitario verifica el buen funcionamiento de una función en particular, mientras que un test de integracion verifica el resultado de utilizar multiples funciones en conjunto. El test unitario detectó primero el bug de carrito_total.  
...
> R:Es peligroso porque al escribir fuera de los limites del array, se puede estar sobre escribiendo en memoria asignado a variables o estructuras del programa. Esto se debe a que C no verifica los límites de la memoria asignada, solo escribe en memoria contigua dejando el cuidado al programador, mientras que Python o Java si verifican que no exista buffer overflow. 
...
> R:Los test permiten encontrar errores mucho más facilmente que simplemente mirando el código(donde tranquilamente se pueden pasar por alto), sobre todo en los test de integracion cuando se evalua el funcionamiento de multiples funciones. Además, se pueden correr multiples veces en segundos, viendo como evoluciona el resultado ante cambios.
...
> R: EL primer test pasó a pesar del bug porque al tener cantidad 1, el valor se mantenía correcto. El error de la función se daba cuando para un mismo producto se seleccionaban mas de una unidad, ya que la misma no multiplicaba el valor por la cantidad de unidades. 
